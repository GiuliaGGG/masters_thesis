...
```r
kansas %>% 
  select(year, qtr, year_qtr, state, treated, gdp, net_income_margin) %>% 
...
```r
df_clean %>% 
  select(time_numeric, ticker, boycotted, net_income_margin) %>% 
  filter(ticker == "COKE" & time_numeric >= 2010.25 & time_numeric < 2025.75) 
```
### Synth
...
df_clean <- df_clean %>%
  filter(time_numeric >= 2010.25 & time_numeric < 2025.75) 

library(augsynth)
syn <- augsynth(net_income_margin ~ boycotted, company_id, time_numeric, df_clean,
                progfunc = "None", scm = T)
...
```r
asyn <- augsynth(net_income_margin ~ boycotted, company_id, time_numeric, df_clean,
                progfunc = "Ridge", scm = T)
```
...
desyn <- augsynth(net_income_margin ~ boycotted,
                    company_id, time_numeric, df_clean,
                   progfunc = "none", scm = T,
                   fixedeff = T)
```
